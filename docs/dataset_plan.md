...
Tracked annotation source exports:

```text
backend/app/data/slots/fci_day_annotations.json
backend/app/data/slots/fci_night_annotations.json
backend/app/data/slots/faie_day_annotations.json
backend/app/data/slots/faie_night_annotations.json
```
...
FAIE day: B1-B40 display slots, B1-B16 and B24-B31 monitored slots
FAIE night: B1-B40 display slots, B1-B15 and B24-B26 monitored slots
```

FCI day uses left-to-right displayed lane order: `A1-A6` top-left, `A7-A25`
first main lane, `A26-A47` return main lane, `A48-A65` lower-left, and
`A66-A78` lower-right.

The FAIE displayed numbering is shared by day and night: `B1-B17` follows the
main curb, `B18-B23` is the angled section, and `B24-B40` is the final row
numbered from right to left. Runtime JSON includes polygons only for monitored
spaces visible in each variant.

FCI runtime polygons are selected by video variant. `fci_day_slots.json` and `fci_night_slots.json` are generated by `backend/app/tools/generate_fci_slots.py` from their matching annotation exports. If FCI is retaken from a different angle, regenerate the matching day or night runtime slot file.

FAIE runtime polygons are also selected by video variant. `faie_day_slots.json` and `faie_night_slots.json` are generated by `backend/app/tools/generate_faie_slots.py` from their matching annotation exports.
